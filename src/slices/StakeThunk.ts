...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const hecContract = new ethers.Contract(addresses[networkID].NOVA_ADDRESS as string, ierc20Abi, signer);
    const shecContract = new ethers.Contract(addresses[networkID].SNOVA_ADDRESS as string, ierc20Abi, signer);
    const oldshecContract = new ethers.Contract(addresses[networkID].OLD_SNOVA_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
