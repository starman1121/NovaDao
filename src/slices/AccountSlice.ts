...
export const getBalances = createAsyncThunk(
  ...
  async ({ address, networkID, provider }: IBaseAddressAsyncThunk) => {
    ...
    const shecContract = new ethers.Contract(addresses[networkID].SNOVA_ADDRESS as string, ierc20Abi, provider);
...
export const loadAccountDetails = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk) => {
    ...
    const shecContract = new ethers.Contract(addresses[networkID].SNOVA_ADDRESS as string, sHECv2, provider);
    ...
    const oldshecContract = new ethers.Contract(addresses[networkID].OLD_SNOVA_ADDRESS as string, sHECv2, provider);
